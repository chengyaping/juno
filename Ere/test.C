...
void fft(double a[], double b[], int ntot, int n, int nspan, int isn)
{
/* ----------------------------------------------------------------
...
//  it's surely enough if n <= 2^32, and minimal factor is 2
  ...
  for ( int index=1; index<nf; index++)  nfac[index]=0;
...
//  find maximum prime factor
  for ( int index=1; index<nf; index++)
  { //cout<<"debug _nf  "<<nf<<"  Index "<<index<<endl;
    if ( nfac[index] > maxf) { maxf=nfac[index]; }
  }
...
L100:
    ...
    sd=sin(sd+sd);
    kk=0;
    //cout<<"debug _i before"<<i<<endl;
    i++;
   // cout<<"debug _i "<<i<<endl;
...
L998:
      delete [] np; delete [] at; delete [] bt; delete [] ck ; delete [] sk;
      isn=0;
      cout <<"Error: array bounds exceeded within subroutine fft.\n";
      exit(1);
}
...
b[i]=0;
a[i]=gRandom->Gaus(0,1);

}

fft(a,b,1000,1000,1000,1);
for(int i=0;i<1000;i++){
//cout<<a[i]<<"   "<<b[i]<<endl;
}
